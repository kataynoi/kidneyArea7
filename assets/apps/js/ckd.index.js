$(document).ready(function(){
    ...
    ckd.ajax = {
        ...
        },get_ckd: function (items, cb) {
            var url = '/reports/get_ckd',
                params = {
                    items: items
                }
...
ckd.get_ckd = function(items){
        $('#control_bp_list > tbody').empty();
        ckd.ajax.get_ckd(items, function (err, data) {
                ckd.set_ckd(data);
...
ckd.set_ckd=function(data){
        $('#tbl_ckd_list > tbody').empty();
        var no=1;
        if (_.size(data.rows) > 0) {
            _.each(data.rows, function (v) {

                $('#tbl_ckd_list > tbody').append(
                    '<tr>' +
                        '<td>' + no + '</td>' +
                        '<td>' + v.cup_code + '</td>' +
                        '<td>' + v.off_name + '</td>' +
            ...
            $('#tbl_ckd_list > tbody').append('<tr><td colspan="8">ไม่พบรายการ</td></tr>');
        }
